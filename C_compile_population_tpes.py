        ...
        for subdir, dirs, files in os.walk(rootdir):
            for file in sorted(files):
                if file == 'tpes.xlsx':
                    ...
                    df_single_lesion = pd.read_excel(excel_input_file_per_lesion, sheet='TPEs_validated')
                    try:
                        # other sheets: Trajectories, Angles, Areas
                        df_single_trajectories = pd.read_excel(excel_input_file_per_lesion, sheet='Trajectories')
                        df_angles = pd.read_excel(excel_input_file_per_lesion, sheet='Angles')
                        df_areas_IREs = pd.read_excel(excel_input_file_per_lesion, sheet='Areas')
                    except Exception:
                        pass
                    ...
                    #             lambda x: 'IRE-' + patient_id + '-L' + str(x))
                    # except Exception as e:
                    #     pass
                    #     # the stuff below is for MAVERRIC
                    #     print(repr(e))
                    #     print("Path to bad excel file:", excel_input_file_per_lesion)
                    #     continue
...
result = pd.concat(frames_TPEs, ignore_index=False)
