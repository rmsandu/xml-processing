...
- using Shoelace formula to determine the area of a simple polygon whose vertices are described by their Cartesian coordinates in the plane

"""

def area_shoelace_1(p):
...
def area_shoelace_2(target_points_x, target_points_y):
    return abs(sum(target_points_x[i] * (target_points_y[i + 1] -
                                         target_points_y[i - 1])
                       for i in range(-1, len(target_points_x) - 1)))
...
def compute_areas_needles(df_IREs, patientID, Areas_between_needles):
    ...
    list_lesion_count = df_IREs['LesionNr'].tolist()
    ...
    for i, lesion in enumerate(lesion_unique):
        ...
        # TODO: order the vertices clockwise so that they are forming an intersecting polygon (formula area wont work)
        area_planned = area(PlannedTargetPoint)/100 # convert from mm_square to cm_square
...
if __name__ == '__main__':
    ...
    p = [[1,1], [1,4], [2,3]]
    print('shoelace_p1:', area_shoelace_2(target_points_x, target_points_y))
    print('shoelace_p2:', area_shoelace_1(p))
